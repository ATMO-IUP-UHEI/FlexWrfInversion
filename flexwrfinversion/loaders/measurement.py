...
class MeasurementLoaderTotalAndCO2_ff(FlexibleMeasurementLoaderTotal):
    """Measurement loader that contains the total CO2 and the CO2_ff
    ...
    def __init__(
        ...
        measurement_file_city: str | Path,
        measurement_file_germany: str | Path,
        measurement_file_co2_ff: str | Path,
        leave_out: list[str] = None,
        keep_only: list[str] = None,
        times_of_day: list[int] = None,
        ppm_noise: float = None,
        total_sector_key: str = "CO2_TOTAL",
        co2_ff_sector_key: str = "CO2_FF",
    ):
        """Measurement loader that contains the total CO2 and the CO2_ff
        data e.g. from 14C measurements.
        ...
        Args:
            ...
            measurement_file_germany (str | Path): Measurement/concentration file for
                 Germany.
            measurement_file_co2_ff (str | Path): Measurement/concentration file
                 for the CO2_ff data. Expects a field with the
                 CO2_ff data with dimension `measurement_id` and a coordinate
                 containing time information in the dimension "MTime". Exects data
                 variable specified in `co2_ff_sector_key`.
            ...
            total_sector_key (str, optional): The key for the total CO2 sector. Defaults
                 to "CO2_TOTAL".
            co2_ff_sector_key (str, optional): The key for the CO2_ff sector. Defaults to
                 "CO2_FF".
        """
        super().__init__(
            target_loader,
        ...
        self._measurement_file_co2_ff = measurement_file_co2_ff
        self.co2_ff_sector_key = co2_ff_sector_key
    ...
    def _adjust_co2_ff_measurement_coords(
        ...
        """Adjust the coordinates of the CO2_ff measurements to be able to combine
        them with the total measurements.

        Args:
            co2_ff_measurements (xr.DataArray): CO2_ff
                 measurements.
            start_measurement_id (int): The starting measurement ID for the CO2_ff
                 measurements. Should be equal to the number of total measurements.
            mplace_name (str): The name to assign to the MPlace coordinate for the
                 CO2_ff measurements.
        Returns:
            xr.DataArray: CO2_ff measurements with adjusted coordinates.
        """
        new_measurement_coords = (
            np.arange(co2_ff_measurements.measurement_id.size) + start_measurement_id
        )
        mpalce_values = np.char.encode(
            np.array(
                [mplace_name] * co2_ff_measurements.measurement_id.size,
                dtype="str",
            )
        )
        return co2_ff_measurements.rename(measurement_id="measurement").assign_coords(
            measurement=new_measurement_coords,
    ...
    def measurements(self):
        if self._combined_measurements is None:
            ...
            co2_ff_measurements = self._adjust_co2_ff_measurement_coords(
                xr.open_dataset(self._measurement_file_co2_ff)[self.co2_ff_sector_key],
                start_measurement_id=total_measurements.measurement.size,
                mplace_name=self.CO2_FF_MPLACE_NAME,
            )
            self._combined_measurements = (
                xr.concat(
                    [total_measurements, co2_ff_measurements],
                    dim="measurement",
                )
                .astype(FLOAT_PRECISION)
                .compute()
